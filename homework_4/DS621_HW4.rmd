...
# Remove the INDEX column from both training and evaluation datasets, as it is of no value.
training_set <- training_set[,setdiff(colnames(training_set), 'INDEX')]
evaluation_set <- evaluation_set[,setdiff(colnames(evaluation_set), 'INDEX')]
...
```{r, trainingDataExplorationTransformation, echo=FALSE}

# Convert TARGET_FLAG from integer to factor
training_set$TARGET_FLAG<-as.factor(training_set$TARGET_FLAG)
...
```{r, evalDataExplorationTransformation, echo=FALSE}
# Remove dollar signs from the INCOME, HOME_VAL, BLUEBOOK, and OLDCLAIM columns by replacing them with empty strings.
evaluation_set$INCOME <- as.numeric(gsub("[\\$,]", "", evaluation_set$INCOME))
evaluation_set$HOME_VAL <- as.numeric(gsub("[\\$,]", "", evaluation_set$HOME_VAL))
evaluation_set$BLUEBOOK <- as.numeric(gsub("[\\$,]", "", evaluation_set$BLUEBOOK))
evaluation_set$OLDCLAIM <- as.numeric(gsub("[\\$,]", "", evaluation_set$OLDCLAIM))

# Convert spaces to underscores in the EDUCATION, JOB, CAR_TYPE, and URBANICITY columns.
evaluation_set$EDUCATION <- gsub(" ", "_", evaluation_set$EDUCATION)
evaluation_set$JOB <- gsub(' ', '_', evaluation_set$JOB)
evaluation_set$CAR_TYPE <- gsub(' ', '_', evaluation_set$CAR_TYPE) 
evaluation_set$URBANICITY <- gsub(' ', '_', evaluation_set$URBANICITY)

# Remove '<', and 'z_' characters from the MSTATUS, SEX, EDUCATION, JOB, CAR_TYPE, and URBANICITY columns.  
evaluation_set$MSTATUS <- gsub("[<,z_]", "", evaluation_set$MSTATUS)
evaluation_set$SEX <- gsub("[<,z_]", "", evaluation_set$SEX)
evaluation_set$EDUCATION <- gsub("[<,z_]", "", evaluation_set$EDUCATION)
evaluation_set$JOB <- gsub("[<,z_]", "", evaluation_set$JOB)
evaluation_set$CAR_TYPE <- gsub("[<,z_]", "", evaluation_set$CAR_TYPE) 
evaluation_set$URBANICITY <- gsub("[<,z_]", "", evaluation_set$URBANICITY)
...
#function to prepare dataframe for modeling
...
evaluation_set = prepare_data(evaluation_set)
...
```{r, dataExplorationSummary2, echo=FALSE}
# Summarize the training dataset.
summary(training_set)
```
...
We created a base model containing all the variables in the dataset to see which variables are significant.

```{r baseModel, echo = FALSE}
# Disable scientific numbers for readability purposes.
options(scipen = 999)

base_model <- glm(TARGET_FLAG ~ ., family = binomial(link = 'logit'), data = training_set)
summary(base_model)
``` 
...
In this model, we removed variables that appeared to be insignificant to see if this resulted in a better performing model.
...
model_five <- lm(TARGET_AMT^(l) ~ . , data = data)

summary(model_five)
```

Looking at the residual graphs to see if it is normally distributed.

```{r}
ggplot(data = model_five, aes(x = .fitted, y = .resid)) +
  geom_point() + geom_hline(yintercept = 0, linetype = "dashed") +
  geom_smooth(se = FALSE) + xlab("Fitted values") + ylab("Residuals")

ggplot(data = model_five, aes(x = .resid)) + geom_histogram() + xlab("Residuals")

ggplot(data = model_five) + stat_qq(aes(sample = .stdresid)) + geom_abline()
```
\clearpage
