...
# Generate a model that contains all predictors.
baseline_model <- glm(TARGET_FLAG ~., data = training_set, family = 'binomial')
...
In this model, we removed variables that appear to be insignificant.

```{r model2, echo = FALSE}
# Disable scientific numbers for readability purposes.
options(scipen = 999)

model_2 = glm(TARGET_FLAG ~ INCOME + PARENT1 + HOME_VAL + MSTATUS + EDUCATION + TRAVTIME + CAR_USE + TIF + CAR_TYPE + REVOKED + URBANICITY, data = training_set, family = 'binomial')
summary(model_2)
```
