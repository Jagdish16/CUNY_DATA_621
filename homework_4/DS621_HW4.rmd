...
```{r}
# applying boxcox method on the data set with rows that had an accident
...
# create model applying lambda to target variable
model_five <- lm(TARGET_AMT^(l) ~ . -TARGET_FLAG, data = training_set %>% filter(TARGET_FLAG == 1))
summary(model_five)
